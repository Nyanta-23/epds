...
interface PostpartumActionFormProps {
  process: boolean;
  action: () => void;
}

export default function PostpartumActionForm({ process, action }: PostpartumActionFormProps) {
    ...
    <div className="flex items-center justify-start space-x-3 pt-6">
      {process ? (
        ...
        <ButtonConfirmation
          content={{
            title: 'Are you sure?',
            description: 'This will create and publish the postpartum.'
          }}
          onConfirm={action}
        >
          <Button
