...
class PostpartumVisitController extends Controller
{
  ...
  public function index(Request $request)
  {
    ...
    $preset = $request->input('preset', 'all');  // all|week|month|year|custom
    ...
    if ($preset !== 'all' && $preset !== 'custom') {
      [$startDate, $endDate] = match ($preset) {
        ...
        'month' => [now()->subMonth()->toDateString(), now()->toDateString()],
        'year' => [now()->subYear()->toDateString(), now()->toDateString()],
        default => [null, null],
      };
    }
    ...
    $filters = [
      ...
      'risk' => $request->input('risk', 'all'),   // all|normal|low|high
      'preset' => $preset,
      'filter_list' => [
        'date_filter' => [
