...
class PostpartumScheduleService
{
  public function getScheduleForMother($motherId): PostpartumScheduleResponse
  {
    ...
    if ($hoursSinceBirth >= 6 && $hoursSinceBirth <= 72) {
    ...
    } elseif ($hoursSinceBirth > 72 && $hoursSinceBirth <= 168) {
      $currentVisitTarget = 2;
      $label = 'Kunjungan Nifas 2 (KF 2)';
      $nextVisitLabel = 'Kunjungan Nifas 3 (KF 3)';
      $deadlineDate = $birthDate->copy()->addHours(168);
      $nextPhaseStartDate = $birthDate->copy()->addHours(168);
    } elseif ($hoursSinceBirth > 168 && $hoursSinceBirth <= 672) {
      $currentVisitTarget = 3;
      $label = 'Kunjungan Nifas 3 (KF 3)';
      $nextVisitLabel = 'Kunjungan Nifas 4 (KF 4)';
      ...
      $currentVisitTarget = 4;
      $label = 'Kunjungan Nifas 4 (KF 4)';
    ...
    $hasFilled = PostpartumVisit::where('mother_id', $motherId)
      ...
      ->whereDate('created_at', '>=', $birthDate->format('Y-m-d'))
      ->exists();
